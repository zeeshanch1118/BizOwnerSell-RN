...
export default class RecommendedBrokers extends Component {
  constructor(props) {
    super(props)
    this.state = {
      allSimilarBroker: [],
  ...
  //   try {
  //     this.setState({allSimilarBroker: this.props.similarBroker})
  //     console.log('SSSs', this.props.similarBroker)
          ...
          {this.state.loader ? (
            <>
              <div className='col-md-12 mx-auto py-5' id='broker-slider'>
                <Slider {...settings}>
                  {this.props?.similarBroker?.map((item, index) => (
                    <Link to={`/search-for-broker/${item.id}`} key={index}>
                          ...
                          >
                            <div className='card total-listing-section-broker mx-md-3'>
                              <div style={{height: '13rem'}}>
                                <img
                                  src={
                                    item.broker_images[0]?.full_path +
                                    'thumb/' +
                                    item.broker_images[0]?.file_name
                                  }
                                  style={{height: '13rem'}}
                                ...
                                <p className='card-text location-text card-heading'>
                                  <span className='me-2 '>
                                    <img
                                      src={locationIcon}
                                      alt=''
                                      className='d-inline mb-1 w-11px'
                                    />
                                  </span>
